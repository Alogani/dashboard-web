        ...
        if let Some((username, timestamp)) = AUTH_CACHE.read().await.get(&cache_key) {
            if timestamp.elapsed() < Duration::from_secs(CACHE_TTL) {
                ...
                if app_config.is_subdomain_allowed(subdomain, Some(username)) {
                    tracing::debug!(
                        "Cache hit: User {} is allowed for subdomain {}",
                        username,
                        subdomain
                    );
                    ...
                    tracing::debug!(
                        "Cache hit: User {} is NOT allowed for subdomain {}",
                        username,
                        subdomain
                    );
                    return (
    ...
    if let Ok(username) = get_user_from_cookie(&state, cookies).await {
        ...
        if let Some(token) = auth_token {
            ...
            AUTH_CACHE.write().await.insert(
                cache_key,
                ("__unauthenticated__".to_string(), Instant::now()),
            );
        }
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if before_count != after_count {
                tracing::debug!(
                    "Auth cache cleanup: removed {} entries",
